...
bool readTransform(string  s, vector<cv::Mat>& rvecs, vector<cv::Mat>& tvecs)
{
...
void readPara()
{
	...
	const string outputFileName = "images_pattern\\out_camera_data.xml";	//校正資訊
	const string outputFileName2 = "images_pattern\\camera_data.xml";	//對Y軸22.5度轉一圈的R和T
	...
	Mat qview,qview2;
	vector<Point3f>  newpoints, newpoints2;
	Mat newMatrix, newMatrix2;
	Mat distorted_points2d, distorted_points2d2;
	Scalar cc3 = cv::Scalar(0, 0, 255, 255); //BGRA
	int cBlockSize = 5;

	qview = imread("images\\test0.jpg");
	qview2 = imread("images_pattern\\test-22.5.jpg");
	...
	if (!readTransform(outputFileName2, rvecs2, tvecs2))//取得每個Camera的rotation 和 transform
	{
	...
	//projectPoints(Mat(newpoints), rvecs[0], tvecs[0], newMatrix, distCoeffs, distorted_points2d);
...
int mymain(int argc, char* argv[])
{
	help();
	Settings s;
	const string inputSettingsFile = argc > 1 ? argv[1] : "images\\default.xml";
	...
	if (s.inputType == Settings::IMAGE_LIST && s.showUndistorsed)
	{
		...
		int i = 0;
		{
			view = imread("images\\captured.png", 1);
...
// Print camera parameters to the output file
bool readCameraParams(string  s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
